...
import { CiUser, CiDeliveryTruck, CiShoppingTag } from "react-icons/ci";
...
const Header: React.FC = () => {
  ...
  return (
    <>
      <TopHeader />
      <div
        className={`flex items-center font-mont md:flex-col justify-between p-2 z-80 ${
          stick ? "fixed top-0 duration-500 ease-in" : ""
        } bg-white w-full`}
      >
        {/* mobile view */}
          ...
          {isOpen && (
            <div className="w-80 fixed bg-white -top-28 -left-0 bottom-0 z-50 flex flex-col items-center justify-center">
              <div className="flex items-center justify-between w-full">
                ...
                <div className=" fixed -top-0 right-4 bg-black p-[10px]">
                  <IoCloseOutline
              ...
              <div className="Flex flex-row justify-between items-center w-full bg-white">
                ...
                <ul>
                  {navItems.map((item) => (
                    <li
                      className="flex items-center justify-between border-b border-[#D9D9D9]"
                      key={item.path}
                    >
                      <a
                        className="text-black hover:text-[#78909C] p-4"
                        href={item.path}
                      >
                        {item.name}
                      </a>
                ...
                <div className="flex items-center justify-between border-b border-[#D9D9D9]">
                  <div className="flex gap-2 items-center px-2 py-4">
                    <CiUser className="text-black hover:text-[#78909C] text-lg hover:text-2xl duration-500" />
                    Login / Register
                  </div>
                </div>
